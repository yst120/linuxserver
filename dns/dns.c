...
int dns_create_question(struct dns_questions* question, const char* hostname) {
	if (question == NULL || hostname == NULL) return -1;
	memset(question, 0, sizeof(struct dns_questions));
...
int dns_build_request(struct dns_header* header, struct dns_questions* question, char* request, int rlen) {	
...
int dns_client_commit(const char* domain) {
	...
	struct dns_questions question = { 0 };
